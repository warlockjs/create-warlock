...
/**
 * `bun` is in ALLOWED_PACKAGE_MANAGERS and advertised by `--help`, but neither
 * probe looked for it — so `--pm=bun` passed the spelling check and then could
 * never appear among the options meant to honour it. The async detector gets
 * its own case because it CACHES, so a fix applied only to the sync probe is
 * dead code the moment `detectPackageManagers()` has been awaited.
 */
describe("bun is probed like every other allow-listed manager", () => {
  it("offers bun from the sync probe when the machine has it", async () => {
    setInstalled(["npm", "yarn", "pnpm", "bun"]);
    const pm = await loadModule();
  ...
  it("offers bun from the ASYNC detector, which is the one that fills the cache", async () => {
    setInstalled(["npm", "bun"]);
    const pm = await loadModule();
  ...
  it("probes exactly the allow-list minus npm, so the two can never drift apart", async () => {
    setInstalled(["npm", "yarn", "pnpm", "bun"]);
    const pm = await loadModule();
    ...
    const probed = new Set(
      execMock.mock.calls.map(call => String(call[0]).split(" ")[0]),
    );
